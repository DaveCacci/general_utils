...
def convert_u_to_pwm(u, u_max, dt: int, pump_dose_per_minute = 25, period: int = 15):
    ...
    '''
    try:
        pump_dose_per_period = pump_dose_per_minute * period/60 # In mL/period_sec; 4L/h = 66.67mL/min = 1.11mL/s; 1.5L/h = 25mL/min = 0.42mL/s

        required_volume = u/(24*60*60)*dt # In mL
        ...
        if on_periods_tot > 0: 
            dosing_interval = np.round(periods_in_dt/on_periods_tot, 0) # Divide control interval in equal 'on_minutes_tot' fractions
            on_periods = 1 # Turn on the pump for one minute/control interval fraction
            off_periods = dosing_interval - 1
        else:
            dosing_interval = periods_in_dt # If 'saturationLow', don't turn on pumps for any minute in the control interval
            on_periods = 0
            off_periods = dosing_interval

        on_seconds = max(0, min(on_periods*period,86400)) # In seconds
        off_seconds = max(0, min(off_periods*period,86400)) # In seconds
        conversion_error = 'No'
    except Exception as e:
        conversion_error = e
        logging.error(e)
    return (on_seconds, off_seconds), on_periods_tot, conversion_error